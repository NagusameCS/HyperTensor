...
\paragraph{Depth-sink shortcut.}  Reloading the full geometry cache on a
70B model is not free; we want only to verify the cache is not stale.
The runtime defines the \emph{depth-sink} as the single layer where the
cumulative-explained-variance curve of the residual stream flattens to
within $10^{-3}$ of its plateau.  On the four models we have inspected,
this is layer $\sim 2/3$ of the stack (21/32, 19/26, 22/32, 22/32).  The
cache integrity check reads only the depth-sink layer's spectrum and
recomputes it on the fly ($<200$~ms on Llama-3.1-8B).  The two-thirds rule
is a property of these four models, not a theorem.
...
\paragraph{Decayed Oja update.}  For layer $\ell$ with basis
$W\in\R^{k\times d}$ and sample $x=h^\text{tgt}-h^\text{drft}$,
\[
w_i\leftarrow w_i+\eta_t\,x\,(x^\top w_i),\qquad w_i\leftarrow w_i/\norm{w_i}
\quad(i=1,\dots,k),
\]
deflated across rows so the result remains orthonormal.  Learning rate
$\eta_t=\eta_0/\sqrt t$ satisfies Robbins--Monro \cite{robbins1951sa};
default $\eta_0=0.01$.  Convergence to the leading eigenvectors of the
running covariance is classical \cite{oja1982}; what is new is the
\emph{trigger}---only rejection residuals are fed in, by construction the
directions the basis fails to capture.
