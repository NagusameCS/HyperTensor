...
\begin{htnote}
\textbf{Project nomenclature.}  \textit{HyperTensor} is the project;
\textit{Geodesic Projection (GP)} is the inference-time pipeline that
factorises every compressible weight (companion paper);
\textit{Geodesic Runtime Compression (GRC)} is the attention-only subset of
GP and is the empirical anchor for the throughput measurements below;
\textit{OTT/GTC} is the Riemannian-manifold theoretical framework on top
...
\textbf{Meta-Llama-3.1-8B-Instruct Q4\_K\_M} achieves
$\alpha{=}46.9\%$ at $\gamma{=}2$ on an eight-prompt sweep, with
\emph{zero} per-prompt acceptance variance under deterministic decoding,
...
\textit{Meta-Llama-3.1-8B-Instruct Q4\_K\_M} (RTX 4070 Laptop, 32~MB L2,
...
The Llama-8B operating point sits \emph{above} SmolLM2-135M's
...
\emph{accelerate} verification rather than slow it, which is why GRC and
...
skip-O) produced a partial result: at $k{\in}\{128, 256\}$ the geodesic
draft acceptance rate \emph{collapses to $\alpha\approx 0\%$ on most
prompts} (1 of 4 prompts at $k{=}256$ retained $\alpha{=}56.2\%$;
the other 7 ran $\alpha{=}0\%$).  Decode throughput remained
healthy ($31$--$36$ tok/s), so the failure mode is verifier rejection,
not compute starvation.  At $k{=}512$ the cold-cache PCA exceeded a
...
backlog.  The headline number ($\alpha{=}46.9\%$ at $k{=}\infty$, no
GP compression) remains; the negative finding is that
\textbf{geodesic spec-decode and aggressive GP compression do not
compose at low $k$}, which contradicts the multiplicative-composition
hypothesis above and re-opens a calibration question for v0.2: at what
$k$ does the compressed-attention basis still preserve the geodesic
draft's first-token alignment?  Raw partial CSV at
\texttt{docs/figures/paper-c/tv\_of\_k\_sweep\_partial.csv}.

\begin{figure}[h]\centering
\includegraphics[width=0.85\linewidth]{../../docs/figures/paper-c/throughput_vs_rank.png}
...
\label{eq:accept}
\end{equation}
Under \emph{greedy} drafter sampling at temperature $\tau\!\to\!0$, the
...
speculative loop \emph{still emits} the EOS as the accepted token (the
...
under greedy decoding makes any verifier draft over EOS \emph{equivalent
