...
and observe that decode throughput \emph{exceeds} the uncompressed model
...
\textit{HyperTensor} is the umbrella project (compiler, runtime, kernels,
and theory). \textit{Geodesic Projection} (GP) is the overarching
inference-time pipeline that factorises weights along their dominant
geometric directions. \textit{Geodesic Runtime Compression} (GRC), the
...
fine-tuning. \textit{OTT/GTC} (Organic Tensor Topology and the Geodessical
...
\begin{itemize}
\item \textit{Calibration-free.} The construction depends only on the
weights; no text samples, no activation traces, no hyperparameters beyond
$k$.  The cache key is $(\textsf{model SHA-256}, k)$.
\item \textit{Bit-stable.}  Sign canonicalisation removes the
...
\item \textit{Drop-in.}  Wraps any GGUF \cite{ggerganov2023gguf} model with
k-quants \cite{ggerganov2023kquants}; no kernel changes required.
\item \textit{Bytes saved.}  Per layer the Q/K/V working set drops from
...
\emph{improvement} (positive means lower error than vanilla GRC) over
...
exhibits early end-of-sequence on \emph{both} the baseline and the
