...
at a $+13.30\%$ perplexity cost on WikiText-2. We initially attributed the speed-up to L2 cache residency near $k/d{=}0.25$.
...
the L2 hit-rate rises from $7.47\%$ to $12.15\%$ ($+4.68$~pp), consistent
with the geometric prediction that reducing the attention working set to
$\sim 10$~MB moves it inside the $32$~MB L2. The second contributing mechanism
...
Both mechanisms, attention-weight L2 residency and Q/K/V kernel fusion, are
present and consistent with the measured $+6.27\%$. The cache-fit calculation
also provides a capacity bound on the rank at which the fusion path becomes
...
directly shrinks. The $+4.68$~pp improvement is consistent with the
geometric prediction: reducing the attention working set from $\sim 25$~MB
(full $d{=}4096$ Q/K/V) to $\sim 10$~MB (projected $k{=}1024$) moves it
from a regime where it competes with FFN for L2 capacity into one where
there is headroom. We report both numbers in \Cref{tab:ncu-l2} so the
