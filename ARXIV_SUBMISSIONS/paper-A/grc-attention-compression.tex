...
We profile 100 representative decode-phase launches of the residual
\texttt{kernel\_gemv\_q4\_k} (FFN matmul, identical in both runs) with
\texttt{ncu --metrics lts\_\_t\_sectors\_op\_read\_lookup\_hit.sum,
lts\_\_t\_sectors\_op\_read.sum} \cite{nvidia2022nsight}.
The mean L2 read hit-rate is $7.8\%$ in the uncompressed baseline and
$5.7\%$ under GRC at $k{=}1024$ ($\Delta{=}{-}2.1$~pp).
The drop is consistent with the cache-fit story rather than refuting it:
GRC's $k{=}1024$ run installs a $128$~MB $P_t$ and a $72$~MB $W_\text{proj}$
persist hint into L2 to keep the \emph{compressed attention} working set
resident, which evicts the (uncompressed, $\sim\!17$~MB) FFN weights ---
exactly the trade we are paying for.  The \emph{compressed-attention}
GEMV kernel itself runs only when GRC is active, so a like-for-like
hit-rate comparison on that kernel is not possible; the FFN trace above
quantifies the persist-hint footprint cost.  A full L2 trace on the
attention path (residency curve as a function of $k$) is the obvious
next experiment and is filed under \Cref{sec:future}.
Raw CSVs and the parser are at
\path{docs/figures/paper-a/ncu/} and
\path{scripts/parse_ncu_l2.py}.
...
We do not claim a context-length sweep beyond the 128/256 split:
fresh measurements at 1k/2k/4k contexts require cold-cache GRC compresses
($\sim$30\,min/condition on the test laptop) and are deferred to follow-up
work.  The pack-derived split above is reported as a sensitivity check, not
as a substitute for a real context sweep.
