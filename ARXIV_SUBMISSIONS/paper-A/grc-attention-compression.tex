...
at a $+13.30\%$ perplexity cost on WikiText-2 (absolute values: baseline $6.79$, $k{=}1536$ gives $7.69$; both are well within the normal range for an 8B-class Q4\_K\_M model).\footnote{A relative penalty can be misleading without anchoring to the absolute scale: $+13\%$ on a PPL of $6.79$ yields $7.69$, which remains in good-quality territory. The same relative penalty applied to a model with PPL $50$ would yield $56.5$, a qualitatively different situation.}  We initially attributed the speed-up to L2 cache residency near $k/d{=}0.25$.
A direct Nsight Compute trace (\Cref{sec:cachefit-ncu}) gives a more complete
picture: the GEMV-weighted L2 hit-rate is nearly flat ($17.0\%$ baseline vs.\
$16.3\%$ at $k{=}1024$), but this average is dominated by FFN kernels that GRC
does not touch. When scoped to the GRC-affected attention-projection kernel,
the L2 hit-rate rises from $7.47\%$ to $12.15\%$ ($+4.68$~pp), in the
direction predicted by the cache-fit model. We label this evidence
``consistent'' rather than ``confirmatory'': a single profiling
run with $n{=}1$ does not isolate L2 residency from kernel-mix and
data-layout confounds, and total DRAM read by the affected kernel
rises by ${\sim}9$~MB (basis-matrix traffic), which a naive cache-fit
...
We do not attempt to apportion the $+6.27\%$ throughput gain between the
...
would settle the attribution. The cache-fit calculation also provides a
...
($+4.68$~pp) at $k{=}1024$, confirming that the attention working set ($\sim 10$~MB)
achieves improved L2 residency inside the $32$~MB L2 cache; (b) Q/K/V kernel
fusion replaces three separate \texttt{gemv\_q4\_k} launches with a fused dual-Q8
...
Attention-specific L2 residency improvement is therefore a contributing
mechanism to the super-baseline. It is not the sole mechanism. The second
mechanism is visible elsewhere in the trace, in the kernel mix. The
...
the difference is small, but three kernel launches collapse into one fused
trio, reducing dispatch overhead and scheduler pressure. Both contributions,
attention-weight L2 residency and Q/K/V kernel fusion, are consistent with
the measured $+6.27\%$ decode throughput.
...
\paragraph{(P1) Multi-$k$ NCU sweep with replicates.}
...
Estimated cost: ${\sim}45$~min on the test GPU; harness already exists at
\path{scripts/benchmark_ncu_l2_profile.ps1}, only the $k$ list needs
extending.

\paragraph{(P2) L2 capacity manipulation.}
Use a co-running cache-thrashing kernel sized to evict $\Delta\in\{0,
8, 16, 24\}$~MB of L2. The cache-fit model predicts that the GRC
super-baseline shrinks as $\Delta$ grows and disappears once
$C_\text{L2}-\Delta < S(1024)\approx 10$~MB; the kernel-fusion
mechanism should be unaffected by $\Delta$. Plot $T/T_\text{base}$ vs.\
$\Delta$. A flat curve isolates fusion as the cause; a step at the
predicted $\Delta$ isolates L2 residency.
...
Until P1, P2, and P3 are completed, the published claim is what we
state here: ``the trace is consistent with the cache-fit model and
does not refute it.'' We explicitly do not claim it ``confirms''
the model, because the data are insufficient for that stronger claim.
