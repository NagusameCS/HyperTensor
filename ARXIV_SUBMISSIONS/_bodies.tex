...
\begin{table}[h]\centering\small
\caption{Intrinsic-dimensionality estimates across three open-weight
architectures. The $k_\text{int}$ column is the TwoNN estimator of
\citet{facco2017twonn} as recorded in
\texttt{legacy/axiom\_vis/<model>/phase\{1,4\}\_*.json}; the $k_{95}$
column is the PCA-95\% component count from the same files. The two
estimators disagree by a factor of $\sim 3$, which is expected because
residual-stream activations are concentrated on a low-dimensional
nonlinear manifold embedded in a higher-dimensional linear envelope.}
\begin{tabular}{@{}lrrrrrl@{}}
\toprule
Model & $d$ & $k_\text{int}$ (TwoNN) & $k_{95}$ (PCA) & $k_\text{int}/d$ & Axiom set & Consistency \\
\midrule
SmolLM2-135M  & 576   & 17 & 46 & 2.95\% & 24 / 96 & 0.921 \\
Gemma-4-E2B   & 1{,}536 & 25 & 54 & 1.63\% & 23 / 92 & 0.961 \\
Phi-3.5-mini  & 3{,}072 & 11 & 31 & 0.36\% & 22 / 96 & 0.959 \\
\bottomrule
\end{tabular}
\end{table}

The three TwoNN values $\{11,17,25\}$ span an order of magnitude in $d$
but do not grow with $d$.  This is consistent with the
...
\paragraph{Layer-resolved profile and the attention-sink caveat.}
The single-number summary above hides strong layer-wise structure. We
re-ran PCA-95, TwoNN, and the Levina--Bickel maximum-likelihood
estimator~\cite{levina2004mle} on residual-stream activations of
SmolLM2-135M ($n{=}4096$ token states drawn from WikiText-103, every
layer; artefact \texttt{benchmarks/intrinsic\_dim\_real\_grid.json}).
The MLE-LB estimator gives a per-layer intrinsic dimension in the range
$9$--$13$ with no monotone trend, broadly agreeing with TwoNN. PCA-95
behaves very differently: in $17$ of $31$ layers (mid-stack, layers
$12$--$28$) it collapses to $k_{95}{=}1$ because the top singular value
of the centred activation matrix already accounts for $\geq 99.8\%$ of
the variance. This is the well-known attention-sink direction
\cite{xiao2024streamingllm}, not a low-dimensional manifold: the
residual after removing rank one is high-dimensional but small in norm.
The practical consequence for GP is that any rank chosen in the
cache-fit regime ($k \geq 8$, say) lives entirely in this residual, so
GP allocation cannot be reduced to a single intrinsic-dim number; the
relevant quantity is the per-layer manifold-curvature ratio at the
operating rank, which we measure separately.
...
\label{eq:jacobi}
\end{equation}
Writing $J=(J,\dot J)\in\R^{2k}$ and assembling the
curvature-and-connection block into the $2k\times2k$ generator
$A(\lambda)$, Eq.~\eqref{eq:jacobi} becomes
$\dot{J}=A(\lambda)J$, whose flow is the Magnus expansion
\cite{magnus1954}
\begin{equation}
...
\item Phase~3 (weight-norm proxy): \emph{not supported}.
      We initially expected L2 column norm to track functional
...
support that conjecture ($0/120$ pairs viable at $k{=}32$).  The
...
\begin{abstract}
We present Safe Orthogonal Geodesic Deviation (Safe OGD), a geometric method that prevents activation in a labelled forbidden subspace during language-model concept exploration. The method constructs an orthogonal projector $P_{\mathrm{safe}} = I - Q_f Q_f^T$ where $Q_f$ is an orthonormal basis for the forbidden behavioural subspace estimated from labelled examples, in the same family of constructions as INLP~\cite{ravfogel2020inlp}, RLACE~\cite{ravfogel2022rlace}, and LEACE~\cite{belrose2023leace}. By projecting hidden states onto the safe subspace before OGD exploration, activation in the labelled forbidden subspace is eliminated by construction. The 100\% safety / 0\% TEH numbers reported below are therefore by construction \emph{within $Q_f$}, not a claim about unseen harm modes; see the held-out evaluation in \Cref{sec:safe-ogd-heldout}. We demonstrate this no-leak property at all exploration step sizes $\alpha \in [0.05, 0.30]$ across 25 trials. Multi-step OGD chains with coherence scoring enable iterative concept refinement. The MIKU Creativity Benchmark (MCB) provides automated quantitative creativity scoring.
\end{abstract}
...
\section{Related Work}
Representation editing: LEACE (Belrose et al., 2023), nullspace projection (Ravfogel et al., 2020). Snipe operates on UGT coordinates rather than raw hidden states. Model editing: ROME (Meng et al., 2022), MEMIT (Meng et al., 2023) edit facts. Snipe removes behavioral tendencies. Safety benchmarks: RealToxicityPrompts (Gehman et al., 2020), HarmBench (Mazeika et al., 2024). Our 8-category taxonomy is probed geometrically---coordinate activation rather than output text.
...
\item Output: Generated response with COG metric update if novel
\end{enumerate}
...
Response latency: $\sim$2.3 seconds for cached queries (GTC hit), $\sim$4.1 seconds for novel queries (COG EXPAND with full metric update).
