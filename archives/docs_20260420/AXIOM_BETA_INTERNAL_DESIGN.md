...
This document explains the current Axiom Beta-3 implementation in geodessical, the main performance/quality tradeoffs, and the distance to the ideal Organic Training Theory (OTT) model target.

Last updated: April 18, 2026

## Scope

- Runtime implementation: [runtime/nn/axiombeta.c](runtime/nn/axiombeta.c)
- Supporting math/geometry: [runtime/nn/axiomgeo.c](runtime/nn/axiomgeo.c), [runtime/nn/axiomlinalg.c](runtime/nn/axiomlinalg.c)
- Plan and benchmark log: [docs/GEODESSICALPLAN.md](docs/GEODESSICALPLAN.md)
...
- Uses decode-aligned oracle targets by default (`geodesicuseoracle_target=1`),
...
- Includes warp accumulation and threshold-based recalculation trigger telemetry
	(`warppointsaccumulated`, `warpcrosstermestimate`, `recalctriggered`).
- Includes persistent warp-state storage (`axiomwarpstate.dat`) so accumulation
...
- Emits target-source telemetry (`oracletargetcount`, `randomtargetcount`)
	so decode-aligned vs legacy random-target pilot behavior is measurable.

## Fast-Mode Design (Current)

Current fast-mode policy in [runtime/nn/axiombeta.c](runtime/nn/axiombeta.c):
...
- `enableknowledgeinjection = 0` (kept off on fast path by default)
...
From latest reports and benchmark history in [docs/GEODESSICALPLAN.md](docs/GEODESSICALPLAN.md):
...
Current readiness estimate (engineering, not theorem-proof) --- April 2026:

| Component | Readiness | Notes |
|-----------|-----------|-------|
| Geometry foundation (metric/Christoffel/curvature) | 70% | Warm-cache operational; full FIM pending |
| Axiom discovery (active learning + model oracle) | 65% | MRR improving; formal grammar pending |
| Geodesic inference replacement path | 35% | Pilot only; not decode-path default |
| Knowledge injection by local curvature warp | 55% | Plumbing done; training-time policy pending |
| End-to-end OTT production replacement | 25% | Diffeomorphism not implemented |
| Overall closeness to ideal OTT model | ~70% | Geometry + survey strong; geodesic generation the gap |
...
## Next Engineering Moves (v0.6 Cortex Targets)

1. Promote Phase 5 from evaluator to candidate token proposer in decode loop.
2. LRU hidden-state cache (tokenid  layer -> float[dim]) to eliminate redundant oracle calls across phases (items 21--26 in GEODESSICALPLAN.md).
3. Block-level manifold sampling --- partition vocab into N=8 blocks; reduce Phase 1 sample calls 256->32 (8), Phase 3 calls ~1900->256 (7).
4. Online softmax merge for Phase 5 probe pool scoring --- process probes in chunks of 32 to reduce peak allocation from 2.3 MB -> 72 KB.
5. Depth-sink layer detection --- probe 16 diverse tokens across all L layers; pick minimum-variance layer as default hidden-state capture point.
6. Layer-wise geodesic trajectory matching metrics vs transformer hidden states.
7. Benchmark harness for median/p95 phase timings across 5--10 runs.
