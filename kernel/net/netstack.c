                       ...
                       const struct ip_hdr *iph)
{
    ...
    /* RFC 793: verify TCP checksum; silently discard corrupted segments */
    uint32_t tcp_seg_len = ip_len - hdr_len;
    if (tcp_checksum(iph->src, iph->dst, ip_pkt + hdr_len, tcp_seg_len) != 0) return;
...
static void handle_ip(const uint8_t *frame, uint32_t len)
{
    ...
    /* RFC 1122 §3.2.1.2: discard packets with invalid IP header checksum */
    uint32_t hdr_len_ip = (iph->version_ihl & 0x0F) * 4;
    if (ip_checksum(ip_pkt, hdr_len_ip) != 0) { stat_ip_rx++; return; }
