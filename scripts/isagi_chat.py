...
# Real OTT engine — geodesic flow + jury-gated verification
_sdir = os.path.dirname(os.path.abspath(__file__))
if _sdir not in sys.path:
    sys.path.insert(0, _sdir)
import ott_engine  # noqa: E402
OTTEngine = ott_engine.OTTEngine
JuryDraftGate = ott_engine.JuryDraftGate
GeodesicDraftGenerator = ott_engine.GeodesicDraftGenerator
del ott_engine, _sdir
...
class GTCCache:
    ...
    def query_jury(self, k_proj, raw_text=''):
        if not self.responses:
            self.misses += 1
            return False, None, 0.0, 0.0, 0
        if raw_text and raw_text in self.labels:
            idx = self.labels.index(raw_text)
            self.hits += 1
            return True, self.responses[idx], 1.0, 1.0, 1
        q = F.normalize(k_proj.unsqueeze(0).float(), dim=1)
        emb_stack = torch.stack(self.embeddings).float().to(q.device)
        sims = (emb_stack @ q.T).squeeze(-1)
        N_JURORS = min(7, len(self.responses))
        top_vals, top_idx = torch.topk(sims, k=N_JURORS)
        distances = 1.0 - top_vals
        R_val = self.radius if self._calibrated else 0.5
        R_val = max(R_val, 0.01)
        confidences = torch.exp(-distances / R_val)
        J = (1.0 - torch.prod(1.0 - confidences)).item()
        best_sim = top_vals[0].item()
        best_idx = top_idx[0].item()
        if J >= 0.85 and best_sim >= 0.5:
            self.hits += 1
            return True, self.responses[best_idx], round(J, 4), round(best_sim, 4), N_JURORS
        self.misses += 1
        return False, None, round(J, 4), round(best_sim, 4), N_JURORS
...
# -------------------------------------------------------
#
# OTTSpeculator (legacy alias) → OTTEngine from ott_engine.py
# The real OTT engine uses:
#   - Geodesic flow in PCA subspace (not random temperatures)
#   - Jury-GTC draft gating (skip transformer verify when J > θ)
#   - GRC feedback loop (learn from transformer corrections)
#
# For backward compatibility, OTTSpeculator is an alias for OTTEngine.
OTTSpeculator = OTTEngine
